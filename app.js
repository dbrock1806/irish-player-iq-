...
if(state.learning.schema!==2){state.learning={cohort:[],maintenance:[],mastery:{},round:1,sessionQuestions:0,historyPlayers:[],historyTypes:[],introduced:[],finished:false,schema:2};}else{state.learning.schema=2;}
...
function initLearning(){
 ...
 ROSTER.forEach(p=>{const o=old[p.name]||{};state.learning.mastery[p.name]={number:clamp(o.number),player:clamp(o.player),position:clamp(o.position),class:clamp(o.class),attempts:Number(o.attempts||0),correct:Number(o.correct||0),consecutive:Number(o.consecutive||0),last7:Array.isArray(o.last7)?o.last7.slice(-7):[],good:!!o.good,mastered:!!o.mastered,goodAt:Number(o.goodAt||0),postGood:Object.assign({number:0,player:0,position:0,class:0},o.postGood||{})}});
 if(!Array.isArray(state.learning.cohort)||state.learning.cohort.length!==6){
   ...
   const fresh=ROSTER.filter(p=>!introduced.has(p.name)).slice(0,6);
...
function updateLearningMastery(name,skill,correct){
 ...
 r.attempts++;
 if(correct){r.correct++;r.consecutive++;}else{r.consecutive=0;}
 ...
 const overallNow=Math.round((r.number+r.player+r.position+r.class)/4);
 ...
 const skillReady=['number','player','position','class'].every(k=>r[k]>=90);
 const attemptReady=['number','player','position','class'].every(k=>r[k]>=2);
 if(r.good&&skillReady&&attemptReady&&postGoodReady&&overallNow>=90)r.mastered=true;
...
function chooseNextCohort(){
 ...
 return ROSTER.filter(p=>!introduced.has(p.name)).slice(0,6);
}
...
function chooseLearning(){
 ...
 const r=rec(p.name);
 const needs=[];
 if(r.number<100)needs.push('playerNumber');
 if(r.player<100)needs.push('numberPlayer');
 if(r.position<100)needs.push('playerPosition');
 if(r.class<100)needs.push('playerClass');
...
function nav(){return `<nav class="bottom-nav"><button data-nav="home" class="${state.view==='home'?'active':''}">⌂<span>HOME</span></button><button data-nav="roster" class="${state.view==='roster'?'active':''}">♙<span>ROSTER</span></button><button data-nav="progress" class="${state.view==='progress'?'active':''}">◎<span>PROGRESS</span></button><button data-nav="season" class="${state.view==='season'?'active':''}">◷<span>SEASON</span></button><button data-nav="more" class="${state.view==='more'?'active':''}">•••<span>MORE</span></button></nav>`}
...
function answer(btn){
 ...
 if(correct){
   ...
   if(state.mode==='learning'){updateLearningMastery(state.current.name,q.skill,true);}
   ...
   if(state.mode==='learning'){updateLearningMastery(state.current.name,q.skill,false);}
