...
function playerBioUrl(p){const special={'Koʿo Kia':'koo-kia','Ko’o Kia':'koo-kia'};const slug=special[p.name]||slugifyName(p.name);return `https://fightingirish.com/roster/${slug}/`}
...
function bind(){
 ...
 app.addEventListener('click',event=>{
   const el=event.target.closest('button,a,[data-nav],[data-mode],[data-action],[data-team],[data-player]');
   if(!el || !app.contains(el))return;
   if(el.matches('a[href]') && !el.dataset.nav && !el.dataset.mode && !el.dataset.action && !el.dataset.team && !el.dataset.player)return;
   if(el.dataset.nav){event.preventDefault();state.view=el.dataset.nav;state.mode=null;state.questionData=null;save();render();return;}
   if(el.dataset.mode){event.preventDefault();state.mode=el.dataset.mode;state.view='quiz';state.current=null;state.questionData=null;state.roundQuestions=0;state.locked=false;if(state.mode==='history')state.history={used:[],difficulty:1,questionIndex:0,gameOver:false,correct:0};newQuestion();return;}
   if(el.dataset.action){event.preventDefault();action(el.dataset.action);return;}
   if(el.dataset.player){event.preventDefault();rosterDetail(el.dataset.player);return;}
   if(el.dataset.team){event.preventDefault();openTeam(el.dataset.team);return;}
   if(el.classList.contains('answer')){event.preventDefault();answer(el);return;}
 });
}
